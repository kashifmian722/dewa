...
export default class DewaCheckoutFinish extends Plugin {
    init() {
        ...
        this._cd = setInterval(function () {}, 1000);
    ...
    _drawMap() {
        ...
        this.leaflet = {
            map: L.map('dewaMap', {
                center: [
                    this.el.dataset.locationLat,
                    this.el.dataset.locationLon
                ],
                zoom: 16,
                zoomControl: false
            })
        };
        this.leaflet.map.scrollWheelZoom.disable();

        L.tileLayer(this.OSM.urlTemplate, {
            attribution: this.OSM.attribution
    ...
    _getOrderState() {
        const requestUrl = this.el.dataset.dewaCheckoutFinish;

        this._client.get(requestUrl, (response) => {
            this.orderState = JSON.parse(response);
    ...
    _updateState() {
        const stateContent = document.getElementById('dewaStateContent');
        stateContent.innerHTML = this.orderState.state;
    ...
    _updateMap() {
        ...
        featureMarker.push(L.marker([
            that.orderState.destination.lat,
            that.orderState.destination.lon
        ], {
            icon: that._getSvgIcon('delivery', 'delivery')
        }).bindPopup(that.orderState.destination.popup, {autoPan: false, autoClose: true}));

        featureMarker.push(
            L.marker([
                that.orderState.location.lat,
                that.orderState.location.lon
            ], {
                icon: that._getSvgIcon('location', 'location')
            }).bindPopup(that.orderState.location.popup, {autoPan: false, autoClose: true})
        );

        if (that.orderState.deliverer && that.orderState.deliverer.lat && that.orderState.deliverer.lon) {
            featureMarker.push(
                L.marker([
                    that.orderState.deliverer.lat,
                    that.orderState.deliverer.lon
                ], {
                    icon: that._getSvgIcon('deliverer', 'deliverer')
                }).bindPopup(that.orderState.deliverer.popup, {autoPan: false, autoClose: true})
            )
        }
    ...
    _countdown() {
        ...
        // TODO: Other tidier method to set countdownEndsAt
        const countdownEndsAt = new Date(this.orderState.calculatedTime.date + ' ' + this.orderState.calculatedTime.timezone);
        const now = new Date();

        let countdownLeft = Math.round((countdownEndsAt.getTime() - now.getTime()) / 1000);
        let countDownElements = this.el.getElementsByClassName("dewa-checkout-cd-el");

        clearInterval(this._cd);

        if (countdownLeft > 0) {
            // TODO: Countdown stucks for some seconds
            this._cd = setInterval(function () {
                countdownLeft--;

                let days = Math.floor(countdownLeft / (60 * 60 * 24));
                let hours = Math.floor((countdownLeft % (60 * 60 * 24)) / (60 * 60));
                let minutes = Math.floor((countdownLeft % (60 * 60)) / (60));
                let seconds = Math.floor((countdownLeft % (60)));

                countDownElements[0].innerHTML = zeroPad(days, 2);
                countDownElements[1].innerHTML = zeroPad(hours, 2);
                countDownElements[2].innerHTML = zeroPad(minutes, 2);
                countDownElements[3].innerHTML = zeroPad(seconds, 2);
            }, 1000);
        } else {
            countDownElements[0].innerHTML = zeroPad(0, 2);
            countDownElements[1].innerHTML = zeroPad(0, 2);
            countDownElements[2].innerHTML = zeroPad(0, 2);
            countDownElements[3].innerHTML = zeroPad(0, 2);
        }
    }
}
